...
const TeamFormation: React.FC = () => {
  ...
  return (
    <div className="container mx-auto p-4">
          ...
          <div className="bg-white p-4 rounded w-11/12 lg:w-1/2">
            ...
            <form>
              ...
              <div className="flex justify-end gap-x-4">
                <button
                  className="bg-white text-blue-500 rounded-md border border-blue-500 hover:text-blue-600 hover:border-blue-600 active:opacity-80 py-2 px-4 "
          ...
          <div className="bg-white p-4 rounded w-11/12 lg:w-1/2">
                ...
                teams.map((team) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="bg-blue-500 flex gap-1 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded mx-2"
                    >
                      <IconOctagonPlus />
                      Add to team
                    </button>
            ...
            <div className="flex justify-end p-4">
              <button
                ...
                className="text-indigo-600 hover:text-indigo-500 border border-indigo-600 hover:border-indigo-500 active:border-indigo-700 font-bold py-2 px-4 rounded mt-4"
              >
                Cancel
              </button>
            </div>
          ...
          <div className="bg-white max-h-[500px] max-w-[100%] overflow-scroll p-4 rounded w-11/12 lg:w-1/2">
            ...
            <ul>
              {clickedTeamDetails !== undefined ? (
                <div key={clickedTeamDetails?.id} className="text-sm">
                  <div className="flex justify-between items-center">
                    <p>Country : </p>
                    <p className="text-gray-500">
                      {clickedTeamDetails?.country || "null"}
                    </p>
                  </div>
                  <div className="flex justify-between items-center">
                    <p>Name : </p>
                    <p className="text-gray-500">
                      {clickedTeamDetails?.name || "null"}
                    </p>
                  </div>
                  <div className="flex justify-between items-center">
                    <p>Id : </p>
                    <p className="text-gray-500">
                      {clickedTeamDetails?.id || "null"}
                    </p>
                  </div>
                  <div className="flex justify-between items-center">
                    <p>Player Count : </p>
                    <p className="text-gray-500">
                      {clickedTeamDetails?.playerCount || "null"}
                    </p>
                  </div>
                  <div className="flex justify-between items-center">
                    <p>Region : </p>
                    <p className="text-gray-500">
                      {clickedTeamDetails?.region || "null"}
                    </p>
                  </div>

                  <div className="grid grid-cols-1">
                    {clickedTeamDetails.players.length > 0 && (
                      <table className="w-full mt-4 border-collapse border border-orange-300">
                        <thead className="font-semibold text-sm text-indigo-500">
                          <tr className="">
                        ...
                        <tbody className="text-sm text-indigo-500">
      ...
      <div className="mt-4">
        <h3 className="text-lg font-bold mb-2 text-indigo-500">Teams</h3>
        {teams.length > 0 ? (
          <ul className="">
            {teams.map((team) => (
              <li
                key={team.id}
                className="mb-2 border gap-y-4 border-indigo-500 p-3 rounded-md"
              >
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-y-2">
                  <div className="flex justify-start items-center gap-x-4">
                    <p className="text-indigo-500">Team Name:</p>
                    <p>{team.name}</p>
                  </div>
                  <div className="flex justify-start items-center gap-x-4">
                    <p className="text-indigo-500">Total Player Count:</p>
                    <p>{team.playerCount} Player</p>
                  </div>
                  <div className="flex justify-between items-center">
                    <button
                      onClick={() => openModal(team)}
                      className="bg-green-500 text-green-50 flex gap-x-2 hover:bg-green-700 font-bold py-1 px-2 rounded mx-2"
                    >
                      Edit
                      <IconEdit />
                    </button>
                    <button
                      ...
                      className="bg-red-500 flex gap-x-2 hover:bg-red-700 text-red-50 font-bold py-1 px-2 rounded"
                    >
                      Remove
                      <IconTrash />
                    </button>
                    <button
                      ...
                      className="bg-indigo-500 flex gap-x-2 mx-2 hover:bg-indigo-700 text-indigo-50 font-bold py-1 px-2 rounded"
                    >
                      Details
                      <IconInfoCircle />
                    </button>
